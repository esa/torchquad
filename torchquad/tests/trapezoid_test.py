...
def _run_trapezoid_tests(backend, _precision):
    ...
    for err, test_function in zip(errors, funcs):
        assert test_function.get_order() > 1 or (err < 2e-11 if test_function.is_integrand_1d else err < 5e-10) # errors add up if the integrand is higher dimensional
    for error in errors:
        assert error < 1e-5
    ...
    for err, test_function in zip(errors, funcs):
        assert test_function.get_order() > 1 or (err < 1e-12 if test_function.is_integrand_1d else err < 2e-11) # errors add up if the integrand is higher dimensional
    for error in errors:
        assert error < 6e-3
