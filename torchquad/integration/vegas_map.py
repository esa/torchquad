...
class VEGASMap:
    ...
    def __init__(self, dim, integration_domain, N_intervals=100, alpha=0.5) -> None:
        ...
        self.alpha = alpha  # Weight smoothness?
        self.x_edges = torch.zeros((self.dim, self.N_edges))  # x locations?
        self.dx_edges = torch.zeros((self.dim, self.N_intervals))  # stepsizes
        ...
        self.delta_weights = torch.zeros(self.dim)
        self.avg_weight = torch.zeros(self.dim)
        self.std_weight = torch.zeros(self.dim)
        # numbers of random numbers in specific interval?
