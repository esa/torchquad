...
class QAG(BaseQuadpack):
    ...
    def _integrate_1d(self, domain_1d, epsabs, epsrel, limit=50, key=2, **kwargs):
        ...
        logger.debug(f"QAG 1D integration: [{a}, {b}], epsabs={epsabs}, epsrel={epsrel}, limit={limit}")
        ...
        while subdivision.should_continue():
            ...
            try:
                ...
                res_left, err_left, _, _, neval_left = GaussKronrodRule.evaluate_gk21(f_1d, a_i, c, "numpy")
                subdivision.neval += neval_left
                self._nr_of_fevals += neval_left
                
                # Integrate right subinterval [c, b_i]
                res_right, err_right, _, _, neval_right = GaussKronrodRule.evaluate_gk21(f_1d, c, b_i, "numpy")
            ...
            if subdivision.converged(epsabs, epsrel):
                ...
                logger.debug(f"QAG converged after {iteration+1} subdivisions: result={total_result}, error={total_error}")
                return total_result
        ...
        tolerance = anp.maximum(epsabs, epsrel * anp.abs(total_result))
        if total_error > tolerance:
            logger.warning(f"QAG failed to converge: result={total_result}, error={total_error}, tolerance={tolerance}")
            warnings.warn(f"QAG did not achieve requested tolerance. "
    ...
    def integrate(self, fn, dim, integration_domain=None, backend=None,
                  epsabs=1.49e-8, epsrel=1.49e-8, limit=50, key=2, **kwargs):
