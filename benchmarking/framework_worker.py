...
def get_test_function(backend_name: str):
    ...
    if backend_name == "torch":
        ...
        def tf_func(x):
            ...
            oscillatory = tf.sin(30 * tf.constant(np.pi) * x[:, 0]) * tf.exp(-10 * (x[:, 0] - 0.3) ** 2)
...
def run_backend_benchmark(backend_name: str, device: str, method_name: str, 
            ...
            for run in range(warmup_runs + num_runs):
                ...
                if method_name == "monte_carlo":
                    result = integrator.integrate(
                        test_func, dim=1, N=n_points,
                        integration_domain=domain, seed=42 + run
                    )
                else:
                    result = integrator.integrate(
                        test_func, dim=1, N=n_points,
