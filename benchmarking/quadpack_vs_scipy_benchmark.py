...
def benchmark_single_tolerance(scipy_func, tq_integrator, func, domain, tol, reference_value=None):
    ...
    try:
        ...
        scipy_result, scipy_abserr = integrate.quad(
            ...
            epsabs=tol,
            epsrel=tol,
            limit=50000,  # Allow many more subdivisions for extreme functions
        )
    ...
    print(
        f"    scipy: result={scipy_result:.6e}, error={scipy_error:.3e}, "
        f"time={scipy_time:.3f}s, evals={scipy_eval_count[0]}"
    )
    print(
        f"    tq:    result={tq_result:.6e}, error={tq_error:.3e}, "
...
def get_reference_value(scipy_func, domain, name):
    ...
    try:
        ...
        ref_result, _ = integrate.quad(
            scipy_func, domain[0], domain[1], epsabs=1e-15, epsrel=1e-15, limit=1000
        )
...
def benchmark_all_quadpack_methods():
    ...
    # Test functions designed to show clear convergence progression
    # Each function should require progressively more work as tolerance tightens
    
    def adaptive_oscillatory_scipy(x):
        # Oscillatory function that becomes harder to integrate as tolerance decreases
        # Requires 21-2000+ evaluations depending on tolerance
        return np.cos(20 * np.pi * x) * np.exp(-2 * x) + 0.1 * np.sin(100 * np.pi * x)

    def adaptive_oscillatory_torch(x):
        x_val = x[:, 0] 
        return anp.cos(20 * np.pi * x_val) * anp.exp(-2 * x_val) + 0.1 * anp.sin(100 * np.pi * x_val)

    def challenging_rational_scipy(x):
        # Rational function with challenging behavior near boundaries
        # Difficulty scales with tolerance due to near-singularities
        return 1.0 / (1 + 25 * (x - 0.2)**2) + 1.0 / (1 + 25 * (x - 0.8)**2) + x**0.1 * (1-x)**0.1

    def challenging_rational_torch(x):
        x_val = x[:, 0]
        return 1.0 / (1 + 25 * (x_val - 0.2)**2) + 1.0 / (1 + 25 * (x_val - 0.8)**2) + x_val**0.1 * (1-x_val)**0.1

    def progressive_difficulty_scipy(x):
        # Function designed to require different subdivision levels
        # Easy at loose tolerances, harder at tight tolerances
        return np.exp(-10 * (x - 0.3)**2) * np.sin(50 * x) + np.exp(-20 * (x - 0.7)**2) * np.cos(80 * x) + 0.01 * np.sin(200 * np.pi * x)

    def progressive_difficulty_torch(x):
        x_val = x[:, 0]
        return anp.exp(-10 * (x_val - 0.3)**2) * anp.sin(50 * x_val) + anp.exp(-20 * (x_val - 0.7)**2) * anp.cos(80 * x_val) + 0.01 * anp.sin(200 * np.pi * x_val)
    
    def boundary_layer_scipy(x):
        # Steep boundary layer - requires more subdivisions for tighter tolerances
        steep = 50  # Steepness parameter
        return np.tanh(steep * (x - 0.3)) + np.tanh(steep * (x - 0.7)) + 0.1 * x**2

    def boundary_layer_torch(x):
        x_val = x[:, 0]
        steep = 50
        return anp.tanh(steep * (x_val - 0.3)) + anp.tanh(steep * (x_val - 0.7)) + 0.1 * x_val**2

    # Define much more challenging test functions that require varying numbers of evaluations
    def extremely_oscillatory_scipy(x):
        # Requires 100-50000+ evaluations depending on precision needed
        return np.sin(50 * np.pi * x) * np.exp(-2 * x) + 0.1 * np.sin(500 * np.pi * x) * np.exp(-10 * (x - 0.3)**2)
    
    def extremely_oscillatory_torch(x):
        x_val = x[:, 0]
        return anp.sin(50 * np.pi * x_val) * anp.exp(-2 * x_val) + 0.1 * anp.sin(500 * np.pi * x_val) * anp.exp(-10 * (x_val - 0.3)**2)
    
    def sharp_peaks_scipy(x):
        # Multiple sharp peaks requiring adaptive subdivision
        result = 0.0
        for center in [0.1, 0.3, 0.6, 0.9]:
            result += 20.0 * np.exp(-2000 * (x - center)**2)
        return result + 0.1 * np.sin(100 * np.pi * x)
    
    def sharp_peaks_torch(x):
        x_val = x[:, 0]
        result = anp.zeros_like(x_val)
        for center in [0.1, 0.3, 0.6, 0.9]:
            result += 20.0 * anp.exp(-2000 * (x_val - center)**2)
        return result + 0.1 * anp.sin(100 * np.pi * x_val)
    
    def near_singularity_scipy(x):
        # Near-singular behavior requiring many evaluations
        return 1.0 / np.sqrt(x + 0.001) + 1.0 / np.sqrt(1.001 - x) + np.sin(20 * np.pi * x)
    
    def near_singularity_torch(x):
        x_val = x[:, 0]
        return 1.0 / anp.sqrt(x_val + 0.001) + 1.0 / anp.sqrt(1.001 - x_val) + anp.sin(20 * np.pi * x_val)
    
    def pathological_scipy(x):
        # Combination of difficulties - oscillatory, peaks, near-singular
        osc = np.sin(80 * np.pi * x) * np.cos(120 * np.pi * x) 
        peaks = 10.0 * np.exp(-1000 * (x - 0.25)**2) + 10.0 * np.exp(-1000 * (x - 0.75)**2)
        singular = 0.5 / (np.sqrt(x + 0.01) * np.sqrt(1.01 - x))
        return osc + peaks + singular
    
    def pathological_torch(x):
        x_val = x[:, 0]
        osc = anp.sin(80 * np.pi * x_val) * anp.cos(120 * np.pi * x_val)
        peaks = 10.0 * anp.exp(-1000 * (x_val - 0.25)**2) + 10.0 * anp.exp(-1000 * (x_val - 0.75)**2)
        singular = 0.5 / (anp.sqrt(x_val + 0.01) * anp.sqrt(1.01 - x_val))
        return osc + peaks + singular

    # Test functions for different methods - designed to show convergence progression
    test_functions = {
        "Extremely Oscillatory": (extremely_oscillatory_scipy, extremely_oscillatory_torch, [0, 1], None),
        "Sharp Peaks": (sharp_peaks_scipy, sharp_peaks_torch, [0, 1], None),
        "Near Singularity": (near_singularity_scipy, near_singularity_torch, [0, 1], None),
        "Pathological": (pathological_scipy, pathological_torch, [0, 1], None),
    }
    ...
    # Use max_fevals to control computational effort and create convergence curves
    # This ensures we get actual convergence progression regardless of function difficulty
    max_fevals_levels = [21, 42, 84, 168, 336, 672, 1344, 2688, 5376, 10752, 21504]
    ...
    for method_name, method in methods.items():
        ...
        for func_name, (scipy_func, tq_func, domain, analytical) in test_functions.items():
            ...
            if analytical is None:
                try:
                    reference, _ = integrate.quad(
                        scipy_func, domain[0], domain[1], epsabs=1e-12, epsrel=1e-12
                    )
                except:
                    reference = None
            else:
            ...
            for max_fevals in max_fevals_levels:
                try:
                    ...
                    result = method.integrate(
                        ...
                        dim=1,
                        integration_domain=[domain],
                        epsabs=1e-10,  # Use tight tolerances, let max_fevals control effort
                        epsrel=1e-10,
                        max_fevals=max_fevals,
                        limit=10000,  # Allow more subdivisions for challenging functions
                        backend="torch",
                    )
                    ...
                    if reference is not None:
                        ...
                        error = 1e-10  # Default tight tolerance
                    ...
                    print(
                        f"    max_fevals={max_fevals}: result={float(result):.6e}, error={error:.3e}, time={elapsed_time:.3f}s, evals={evals}"
                    )

                    # Stop if taking too long or method fails
                    if elapsed_time > 15.0:
                        print(f"    Time limit reached at max_fevals={max_fevals}")
                        break

                except Exception as e:
                    print(f"    max_fevals={max_fevals}: FAILED - {str(e)[:100]}")
                    # For QNG, failures are less expected with max_fevals control
                    if len([r for r in function_results if 'FAILED' in str(r)]) > 3:
                        print(f"    Too many failures for {method_name} - stopping")
                        break
                    continue
...
def run_benchmarks():
    ...
    for test_name, scipy_func, tq_func, domain in test_cases:
        ...
        for tol in tolerance_levels:
            result = benchmark_single_tolerance(scipy_func, qags, tq_func, domain, tol, reference)
...
def create_plots(results, save_path="../resources"):
    ...
    for i, (test_name, data) in enumerate(plot_data.items()):
        ...
        scipy_errors_filtered = [
            data["scipy_errors"][j] for j, e in enumerate(data["scipy_evals"]) if e > 0
        ]
    ...
    for i, (test_name, data) in enumerate(plot_data.items()):
        ...
        scipy_errors_time_filtered = [
            data["scipy_errors"][j] for j, t in enumerate(data["scipy_times"]) if t > 0
        ]
        tq_times_ms = [t * 1000 for t in data["tq_times"] if t > 0]
        tq_errors_time_filtered = [
            data["tq_errors"][j] for j, t in enumerate(data["tq_times"]) if t > 0
        ]
    ...
    if max_data_points > 1:
        ...
        for i, (test_name, data) in enumerate(plot_data.items()):
            ...
            scipy_tols = [
                data["tolerances"][j] for j, t in enumerate(data["scipy_time_per_eval"]) if t > 0
            ]
            tq_tols = [
                data["tolerances"][j] for j, t in enumerate(data["tq_time_per_eval"]) if t > 0
            ]

            if scipy_time_per_eval_us and scipy_tols:
                ax3.loglog(
                    scipy_tols,
                    scipy_time_per_eval_us,
                    color=color,
                    marker=marker_s,
                    label=f"{test_name} (scipy)",
                    linestyle="-",
        ...
        for i, (test_name, data) in enumerate(plot_data.items()):
            ...
            scipy_tols = [
                data["tolerances"][j]
                for j, p in enumerate(data["scipy_error_time_product"])
                if p > 0
            ]
            tq_tols = [
                data["tolerances"][j] for j, p in enumerate(data["tq_error_time_product"]) if p > 0
            ]
        ...
        scipy_times_per_eval = [
            data["scipy_time_per_eval"][0] * 1e6 if data["scipy_time_per_eval"] else 0
            for data in plot_data.values()
        ]
        tq_times_per_eval = [
            data["tq_time_per_eval"][0] * 1e6 if data["tq_time_per_eval"] else 0
            for data in plot_data.values()
        ]

        ax3.bar(x - width / 2, scipy_times_per_eval, width, label="scipy", alpha=0.8, color="blue")
        ax3.bar(
            x + width / 2, tq_times_per_eval, width, label="torchquad", alpha=0.8, color="orange"
        )
        ...
        scipy_products = [
            data["scipy_error_time_product"][0] if data["scipy_error_time_product"] else 1e-16
            for data in plot_data.values()
        ]
        tq_products = [
            data["tq_error_time_product"][0] if data["tq_error_time_product"] else 1e-16
            for data in plot_data.values()
        ]
...
def create_quadpack_convergence_plots(method_results, save_path="../resources"):
    ...
    for i, method_name in enumerate(methods):
        for j, func_name in enumerate(functions):
            ...
            max_fevals_used = [d["max_fevals"] for d in data if d["evals"] > 0 and d["error"] > 0]

            if evals and errors and len(evals) == len(errors):
                # All methods: plot error vs evaluations (this fixes x-axis consistency)
                ax.loglog(
                    evals,
                    errors,
                    "o-",
                ...
                if method_name == "QNG":
                    ax.set_title(f"{method_name} - {func_name}\nError vs Function Evaluations\n(Non-adaptive: uses fixed rules)")
                else:
                    ax.set_title(f"{method_name} - {func_name}\nError vs Function Evaluations\n(Adaptive: error-controlled)")
                
                # Set consistent axis ranges for all subplots
                ax.set_xlim(10, 25000)  # Consistent x-axis for function evaluations
                ax.set_ylim(1e-16, 1e2)  # Consistent y-axis for errors
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n" + "=" * 60)
        print("Running benchmarks for challenging functions")
