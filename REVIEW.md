...
- Inline `if infer_backend(x) == "torch": ...` special-cases scattered through a
  kernel are a smell — prefer a named helper (the roadmap is centralizing these in
  `utils/_backend_compat.py`). A hard `ValueError` rejecting JAX/TF (as VEGAS does)
  should read from a single `SUPPORTED_BACKENDS` source of truth, not be duplicated.
...
CLAUDE.md rule 13; vulture is being wired into CI (roadmap C4). The recurring
